...
CREATE TABLE public.contents ( average_rating float8 NOT NULL, review_count int4 NOT NULL, created_at timestamptz(6) NOT NULL, source_id int8 NOT NULL, id uuid NOT NULL, description text NULL, thumbnail_url varchar(255) NULL, title varchar(255) NOT NULL, "type" varchar(255) NOT NULL);

-- Permissions

ALTER TABLE public.contents OWNER TO app;
...
CREATE TABLE public.notifications ( id uuid NOT NULL, receiver_id uuid NOT NULL, title varchar NOT NULL, created_at timestamp NOT NULL, "content" text NOT NULL, "level" varchar NOT NULL, read_at timestamp NULL, CONSTRAINT pk_notifications null);
CREATE INDEX idx_notifications_receiver_id ON public.notifications (receiver_id uuid_ops);
...
CREATE TABLE public.tags ( created_at timestamptz(6) NOT NULL, id uuid NOT NULL, "name" varchar(255) NOT NULL, CONSTRAINT tags_name_key null, CONSTRAINT tags_pkey null);
...
CREATE TABLE public.users ( "locked" bool NOT NULL, created_at timestamptz(6) NOT NULL, id uuid NOT NULL, email varchar(255) NOT NULL, "name" varchar(255) NOT NULL, "password" varchar(255) NULL, profile_image_url varchar(255) NULL, provider varchar(255) NOT NULL, "role" varchar(255) NOT NULL, CONSTRAINT users_email_key UNIQUE (email), CONSTRAINT users_pkey PRIMARY KEY (id), CONSTRAINT users_provider_check CHECK (((provider)::text = ANY ((ARRAY['LOCAL'::character varying, 'GOOGLE'::character varying, 'KAKAO'::character varying])::text[]))), CONSTRAINT users_role_check CHECK (((role)::text = ANY ((ARRAY['USER'::character varying, 'ADMIN'::character varying])::text[]))));
...
-- DROP TABLE public.content_tags;

CREATE TABLE public.content_tags ( created_at timestamptz(6) NOT NULL, content_id uuid NOT NULL, id uuid NOT NULL, tag_id uuid NOT NULL, CONSTRAINT content_tags_pkey null, CONSTRAINT fkcr5rprg24ccjyjsnc7cac859f FOREIGN KEY (tag_id) REFERENCES public.tags(id), CONSTRAINT fkq97jtguli56lt3e73h32jbwxy FOREIGN KEY (content_id) REFERENCES public.contents(id));

-- Permissions

ALTER TABLE public.content_tags OWNER TO app;
...
CREATE TABLE public.follows ( created_at timestamptz(6) NOT NULL, followee_id uuid NOT NULL, follower_id uuid NOT NULL, id uuid NOT NULL, CONSTRAINT follows_pkey null, CONSTRAINT fkeo7hqi2bt2vdwk6mpu0w2ihyb FOREIGN KEY (followee_id) REFERENCES public.users(id), CONSTRAINT fkqnkw0cwwh6572nyhvdjqlr163 FOREIGN KEY (follower_id) REFERENCES public.users(id));

-- Permissions

ALTER TABLE public.follows OWNER TO app;
...
CREATE TABLE public.playlists ( created_at timestamptz(6) NOT NULL, updated_at timestamptz(6) NOT NULL, id uuid NOT NULL, owner_id uuid NOT NULL, description text NULL, title varchar(255) NOT NULL, CONSTRAINT playlists_pkey null, CONSTRAINT fkn0r1rrm19f8j7wfl656bd9sus FOREIGN KEY (owner_id) REFERENCES public.users(id));

-- Permissions

ALTER TABLE public.playlists OWNER TO app;
...
CREATE TABLE public.reviews ( rating float8 NOT NULL, created_at timestamptz(6) NOT NULL, updated_at timestamptz(6) NULL, author_id uuid NOT NULL, content_id uuid NOT NULL, id uuid NOT NULL, "text" text NOT NULL, CONSTRAINT reviews_pkey null, CONSTRAINT fkohsyiuxht0o3qx6hmtytxdyo4 FOREIGN KEY (content_id) REFERENCES public.contents(id), CONSTRAINT fkse5kx11600wtv0jh9jobvrdpi FOREIGN KEY (author_id) REFERENCES public.users(id));

-- Permissions

ALTER TABLE public.reviews OWNER TO app;
...
-- DROP TABLE public.playlist_contents;

CREATE TABLE public.playlist_contents ( created_at timestamptz(6) NOT NULL, content_id uuid NOT NULL, id uuid NOT NULL, playlist_id uuid NOT NULL, CONSTRAINT playlist_contents_pkey null, CONSTRAINT fkjsc6gyglh0w7fn83movab0lq1 FOREIGN KEY (content_id) REFERENCES public.contents(id), CONSTRAINT fknjuw6x7c4ys2l627bl1xocdc9 FOREIGN KEY (playlist_id) REFERENCES public.playlists(id));

-- Permissions

ALTER TABLE public.playlist_contents OWNER TO app;
...
-- DROP TABLE public.playlist_subscriptions;

CREATE TABLE public.playlist_subscriptions ( created_at timestamptz(6) NOT NULL, id uuid NOT NULL, playlist_id uuid NOT NULL, user_id uuid NOT NULL, CONSTRAINT playlist_subscriptions_pkey null, CONSTRAINT fkdf1h4h7vqlvygg4uia944hhk6 FOREIGN KEY (playlist_id) REFERENCES public.playlists(id), CONSTRAINT fkoh6gfn5s6sto9b9cxios0cy24 FOREIGN KEY (user_id) REFERENCES public.users(id));

-- Permissions

ALTER TABLE public.playlist_subscriptions OWNER TO app;
...
CREATE TABLE public.conversation_participants ( id uuid NOT NULL, created_at timestamp NOT NULL, user_id uuid NOT NULL, conversation_id uuid NOT NULL, last_read_at timestamp NULL, CONSTRAINT pk_conversation_participants null, CONSTRAINT uq_conv_participants null);
CREATE INDEX idx_conversation_participants_user_id ON public.conversation_participants (user_id uuid_ops);

-- Permissions

ALTER TABLE public.conversation_participants OWNER TO app;
...
CREATE TABLE public.conversations ( id uuid NOT NULL, created_at timestamp NOT NULL, last_message_id uuid NULL, CONSTRAINT pk_conversations null);

-- Permissions

ALTER TABLE public.conversations OWNER TO app;
...
CREATE TABLE public.direct_messages ( id uuid NOT NULL, conversation_id uuid NOT NULL, sender_id uuid NOT NULL, "content" text NOT NULL, created_at timestamp NOT NULL, read_at timestamp NULL, CONSTRAINT pk_direct_messages null);
...
ALTER TABLE public.direct_messages OWNER TO app;
GRANT ALL ON TABLE public.direct_messages TO app;


-- public.conversation_participants foreign keys

ALTER TABLE public.conversation_participants ADD CONSTRAINT fk_conv_participants_conversation FOREIGN KEY (conversation_id) REFERENCES public.conversations(id) ON DELETE CASCADE;


-- public.conversations foreign keys

ALTER TABLE public.conversations ADD CONSTRAINT fk_conversations_last_message FOREIGN KEY (last_message_id) REFERENCES public.direct_messages(id) ON DELETE SET NULL;


-- public.direct_messages foreign keys

ALTER TABLE public.direct_messages ADD CONSTRAINT fk_dm_conversation FOREIGN KEY (conversation_id) REFERENCES public.conversations(id) ON DELETE CASCADE;
